...
if __name__ == '__main__':
    ...
    url = fileio.uploadEphemeralFile('/tmp/{0}.zip'.format(filename))
    prepare_file.delete_file('/tmp/{0}.zip'.format(filename))
    if url is not None:
        subprocess.run(['osascript', '-e', 'tell application "TextEdit" to make new document with properties {text:"Url: ' + url + '\nKey: ' + key + '"}'])
